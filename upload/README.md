...
Fehlerbehebung bei 404 nach Deployment
-------------------------------------

Wenn du nach dem Push eine GitHub Pages 404-Seite siehst, prüfe bitte:

- Ist die Action gelaufen? Schau unter `Actions` im Repo, ob `deploy-gh-pages` oder `Deploy frontend to GitHub Pages` erfolgreich war.
- Enthält der deployte Branch (`gh-pages`) eine `index.html` in seiner Wurzel? Die `public/index.html` muss in der veröffentlichten Branch vorhanden sein.
- Wenn du eine eigene Domain (CNAME) verwendest: Hast du die DNS-Einträge korrekt gesetzt? Für APEX-Domains (example.com) musst du A/ALIAS/ANAME gemäß GitHub Pages Anleitung setzen; für Subdomains (`www`) setze einen CNAME auf `username.github.io`.
- In den Repo-Einstellungen → Pages: Ist die Source auf den `gh-pages` Branch gesetzt (oder auf GitHub Actions)? Schau auch, ob eine benutzerdefinierte Domain gesetzt ist und ob GitHub eine Warnung zur Validierung anzeigt.

Wenn du möchtest, kann ich die `public/`-Inhalte per Workflow direkt in den `gh-pages`-Branch deployen (bereits hinzugefügt) — das ist oft die zuverlässigste Methode für statische Sites.

DNS-Check-Quicklist
- Wenn du `public/CNAME` mit `example.com` eingerichtet hast, setze diese A-Records beim DNS-Provider: `185.199.108.153`, `185.199.109.153`, `185.199.110.153`, `185.199.111.153` (GitHub Pages IPs) OR verwende ALIAS/ANAME to `username.github.io` if supported.
- Warte ein paar Minuten nach DNS-Änderungen; DNS-Propagation kann eine Weile dauern.
